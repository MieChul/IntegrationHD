...
public class GenericRepository<T> : IGenericRepository<T> where T : BaseEntity
{
    ...
    public async Task<T> GetByDynamicPropertyAsync(string propertyName, object value, bool ignoreCase = false)
    {
        ...
        if (value is string stringValue && ignoreCase)
        {
            var regex = new MongoDB.Bson.BsonRegularExpression(stringValue, "i");
            filter = Builders<T>.Filter.Regex(propertyInfo.Name, regex);
        }
        else
        {
            filter = Builders<T>.Filter.Eq(propertyInfo.Name, value);
        }
